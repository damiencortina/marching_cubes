...
export function marching_cubes_3d_single_cell(f:LevelFunction, x:number, y:number, z:number){
    const levelFunctionResult = VERTICES.map((vertex)=>f(x+vertex[0], y+vertex[1], z+vertex[2]));
